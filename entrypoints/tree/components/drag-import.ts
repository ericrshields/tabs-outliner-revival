 ...
 */
/** Minimal interface matching both native and Preact DataTransfer. */
...
/** Read a File as text, returning a Promise. */
...
/** Decode common HTML entities without DOM manipulation. */
...
function buildHierarchy(nodes: FlatNode[]): HierarchyJSO | null {
  ...
  function buildSubtree(index: number): { jso: HierarchyJSO; nextIndex: number } {
    ...
    while (i < nodes.length && nodes[i].depth > node.depth) {
      // Treat any deeper node as a direct child (clamp depth gaps).
      // Depth gaps occur when the legacy HTML has unusual nesting.
...
/** Detect session root by exact legacy title match. */
function isSessionTitle(title: string): boolean {
  const lower = title.toLowerCase();
  return lower === 'current session';
}
